...
def pde(x, y):
    ...
    v_momentum = (u * dv_x + v * dv_y) + dp_y - Pr * (dv_xx + dv_yy) - Ra * Pr * T # - T
    ...
    u_momentum = u_momentum / (Ra * Pr)
    v_momentum = v_momentum / (Ra * Pr)
    T_energy = T_energy / (Ra * Pr)
...
# 所需输入: data, dt, Nt
# 获得y的最大值和最小值
u_min = data.min(axis=(0, 2,))[3] * uv_star
u_max = data.max(axis=(0, 2,))[3] * uv_star
v_min = data.min(axis=(0, 2,))[4] * uv_star
v_max = data.max(axis=(0, 2,))[4] * uv_star
p_min = data.min(axis=(0, 2,))[5] * p_star
p_max = data.max(axis=(0, 2,))[5] * p_star
T_min = data.min(axis=(0, 2,))[6] * T_star + Tref
T_max = data.max(axis=(0, 2,))[6] * T_star + Tref
...
def init():
    plt.clf()
    x1_t = data[:, 0:1, 0] * xy_star
    x2_t = data[:, 1:2, 0] * xy_star
    xt_0 = 0 * np.ones([data.shape[0], 1]) * t_star
    xinit = np.concatenate([x1_t, x2_t, xt_0], axis=-1)
    u_p_t = ic_u_func(xinit) * uv_star
    v_p_t = ic_v_func(xinit) * uv_star
    p_p_t = ic_p_func(xinit) * p_star
    T_p_t = ic_T_func(xinit) * T_star + Tref
    # print('x1_t: ', x1_t)
    # print('x2_t: ', x2_t)
    # print('u_p_t: ', u_p_t)
    # print('v_p_t: ', v_p_t)
...
def anim_update(t_id):
    plt.clf()
    x1_t = data[:, 0:1, t_id] * xy_star
    x2_t = data[:, 1:2, t_id] * xy_star
    u_p_t = data[:, 3:4, t_id] * uv_star
    v_p_t = data[:, 4:5, t_id] * uv_star
    p_p_t = data[:, 5:6, t_id] * p_star
    T_p_t = data[:, 6:7, t_id] * T_star + Tref
    ...
    plt.xlabel('$x (m)$')
    plt.ylabel('$y (m)$')
    plt.title("u-field at t = " + str(t_id * dt * t_star) + " s.", fontsize = 9.5)
    ...
    plt.xlabel('$x (m)$')
    plt.ylabel('$y (m)$')
    plt.title("v-field at t = " + str(t_id * dt * t_star) + " s.", fontsize = 9.5)
    ...
    plt.xlabel('$x (m)$')
    plt.ylabel('$y (m)$')
    plt.title("p-field at t = " + str(t_id * dt * t_star) + " s.", fontsize = 9.5)
    ...
    plt.xlabel('$x (m)$')
    plt.ylabel('$y (m)$')
    plt.title("T-field at t = " + str(t_id * dt * t_star) + " s.", fontsize = 9.5)

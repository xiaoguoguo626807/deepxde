            ...
            if isinstance(task_name, str) and os.path.exists(f"./{task_name}/linears.{i-1}.weight.npy") and os.path.exists(f"./{task_name}/linears.{i-1}.bias.npy"):
                ...
                self.linears.append(
                    paddle.nn.Linear(
                        layer_sizes[i - 1],
                        layer_sizes[i],
                        weight_attr=ParamAttr(initializer=Assign(np.load(f"./{task_name}/linears.{i-1}.weight.npy"))),
                        bias_attr=ParamAttr(initializer=Assign(np.load(f"./{task_name}/linears.{i-1}.bias.npy")))
                    )
                )
                p += 2
            else:
